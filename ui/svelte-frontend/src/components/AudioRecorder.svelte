<script>
  ...
  function drawMeter() {
    ...
    barColors = bars.map((h, i) => {
      ...
      return val > 0.7 ? 'var(--danger)' : val > 0.4 ? 'var(--accent)' : 'var(--border)'
    })
...
<div id="recorder-container" class="flex">
  <button id="quit-recording" on:click={() => { stopRecording(); dispatch('close') }}>X</button>
  <button class="rec-btn" on:click={startRecording} disabled={recording}>● Rec</button>
  <button class="rec-btn" on:click={stopRecording} disabled={stopped}>■ Stop</button>
  <div class="timer">{timerDisplay}</div>
  <div class="meter-wrap">
    {#each bars as h, i}
      <div class="bar" style="height: {h}%; background: {barColors[i]}"></div>
    {/each}
  </div>
  {#if audioSrc}
